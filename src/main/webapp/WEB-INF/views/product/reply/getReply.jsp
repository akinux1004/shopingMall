	...
	function showList(page){
		...
		replyService.getList({pno : pno, page : page || 1}, function(list){
			...
			for(var i = 0; i < list.length; i++){
				...
				str += '               <button data-oper="modify" data-reply="'+list[i].reply+'" data-rno="'+list[i].rno+'" class="reBtn btn-primary btn-circle text-uppercase"> modify </button>';
				str += '               <button data-oper="update" data-reply="'+list[i].reply+'" data-rno="'+list[i].rno+'" class="reBtn btn-success btn-circle text-uppercase"> success </button>';
				str += '               <button data-oper="remove" data-reply="'+list[i].reply+'" data-rno="'+list[i].rno+'" class="reBtn btn-danger btn-circle text-uppercase"> remove </button>';
				str += '               <a data-rno="'+list[i].rno+'" class="btn_replyOne btn btn-warning btn-circle text-uppercase" data-toggle="collapse" href="#re'+list[i].rno+'re"><span class="glyphicon glyphicon-comment"></span> 2 comments</a>';
				str += '           </div>              ';
				str += '         </div>';
				...
				str += '                           <textarea class="form-control rsTextArea" name="addComment" id="addComment" rows="2"></textarea>';
				str += '                         </div>';
				str += '                     </div>';
				str += '                     <div class="form-group">';
				str += '                         <div class="col-sm-offset-2 col-sm-10">                    ';
				str += '                             <button type="button" data-rno="'+list[i].rno+'" data-replyer="'+list[i].replyer+'" class="rsBtn btn btn-success btn-circle text-uppercase" id="submitComment"><span class="glyphicon glyphicon-send"></span> Summit comment</button>';
				str += '                         </div>';
				str += '                     </div>            ';
				str += '                 </form>';
				str += '             </div><!-- end comment -->';
				...
				str += '             <ul class="media-list replySecond" data-rno="'+list[i].rno+'">';
...
/*  start replySecond */ 			
			$(".rsBtn").on("click", function(){
				...
				var rno = $(this).data("rno");
				var reply = $(this).parent().parent().parent().find(".rsTextArea").val();
				var replyer = $(this).data("replyer");
				var collapse = $(this).parent().parent().parent().parent().parent();
				var collapseList = collapse.parent().parent().find("#large"+rno+"large");
				console.log(collapseList);
				var collapseList1 = collapse.parent().parent().find("#re"+rno+"re");
				console.log(collapseList1);
				replySecond.add({rno : rno, reply : reply, replyer : replyer}, function(result){
					alert(result);
					
					secondList(rno, 1);
					
					collapse.attr("class", "collapse"); //등록후 textarea 숨기기
					
					collapseList1.collapse('show'); // 
			...
			// 버튼 클릭시   1.콜랩스 이벤트와  2.replySecond list 보여주는  이벤트
...
/* end reply second  */			
		...
		}); // end replyService.getList({pno : pno, page : page || 1}, function(list){
...
<script>
function secondList(rnoVal, page){
	str = "";
	
	replySecond.getList({rno : rnoVal , page : page || 1}
		,function(list){ 
			console.log(list);
			...
			for(var i = 0; i < list.length; i++){
				...
				str += '                       <div class="well well-lg">';
				str += '                           <h4 class="media-heading text-uppercase reviews"><span class="glyphicon glyphicon-share-alt"></span>'+list[i].replyer+'</h4>';
				str += '                           <ul class="media-date text-uppercase reviews list-inline">';
				str += '                             <li>[  '+list[i].rno+' : '+list[i].sno+'  ]'+replyService.displayTime(list[i].reg_date)+'</li>';
				str += '                           </ul>';
			...
			//이렇게 rno로 선택자를 지정 해주지 않으면 엄한곳에 댓글 리스트가 달린다.
			$("ul[data-rno='"+rnoVal+"']").html(str);
			
		} // end call back
			
	); // end replySecond.getList

} //end function secondList();
...
<script type="text/javascript" src="/resources/js/reply.js?ver=1"/>
